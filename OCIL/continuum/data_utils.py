...
def create_task_composition(class_nums, num_tasks, fixed_order=False):
    ...
    for tt in range(num_tasks):
        ...
        logging.info('Task: {}, Labels:{}'.format(tt, task_labels[tt]))
...
class dataset_transform(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # candidates=list(np.range(len(self.x)))-idx
        # idx1 =random.choice(candidates)
        # lam=np.random.beta(0.2,0.2)
        # x_blended = self.x[idx]*lam+self.x[idx1]*(1-lam)
        # y_blended = self.y[idx]*lam+self.y[idx1]*(1-lam)
