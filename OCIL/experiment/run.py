...
def multiple_run(params, store=False, save_path=None):
    ...
    data_end = time.time()
    logging.info('data setup time: {}'.format(data_end - start))
    ...
    for run in range(params.num_runs):
        ...
        if params.online:
            for i, (x_train, y_train, labels) in enumerate(data_continuum):
                logging.info("-----------run {} training batch {}-------------".format(run, i))
                logging.info('size: {}, {}'.format(x_train.shape, y_train.shape))
            ...
            # 计算平均TOP-1和TOP-5准确率
            avg_end_acc = np.mean([acc[:, 0] for acc in tmp_acc])  # TOP-1
            avg_end_top5_acc = np.mean([acc[:, 1] for acc in tmp_acc])  # TOP-5

            logging.info("-----------run {}-----------avg_end_acc {} avg_end_top5_acc {}-----------train time {}".format(
                    run, avg_end_acc, avg_end_top5_acc, run_end - run_start))
            ...
            acc_array = agent.evaluate(test_loaders)
            avg_end_acc = acc_array[0]  # Assuming acc_array[0] is TOP-1 accuracy
            avg_end_top5_acc = acc_array[1]  # Assuming acc_array[1] is TOP-5 accuracy
            accuracy_list.append(acc_array)
            print("-----------run {}-----------avg_end_acc {} avg_end_top5_acc {}-----------".format(
                run, avg_end_acc, avg_end_top5_acc))
    ...
    if store:
        ...
        save_file.close()
    if params.online:
        # 提取 Top-1 和 Top-5 准确率
        top1_accuracy_array = accuracy_array[..., 0]  # 提取所有运行中的 Top-1 准确率
        top5_accuracy_array = accuracy_array[..., 1]  # 提取所有运行中的 Top-5 准确率
        logging.info('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
        # 计算 Top-1 准确率的性能
        avg_end_acc_top1, avg_end_fgt_top1, avg_acc_top1, avg_bwtp_top1, avg_fwt_top1 = compute_performance(top1_accuracy_array)
        logging.info(f"Top-1 Avg_End_Acc: {avg_end_acc_top1}, Avg_End_Fgt: {avg_end_fgt_top1}, Avg_Acc: {avg_acc_top1}, "
              f"Avg_Bwtp:{avg_bwtp_top1}, Avg_Fwt: {avg_fwt_top1} ")

        # 计算 Top-5 准确率的性能
        avg_end_acc_top5, avg_end_fgt_top5, avg_acc_top5, avg_bwtp_top5, avg_fwt_top5 = compute_performance(top5_accuracy_array)
        logging.info(f"Top-5 Avg_End_Acc: {avg_end_acc_top5}, Avg_End_Fgt: {avg_end_fgt_top5}, Avg_Acc: {avg_acc_top5}, "
              f"Avg_Bwtp:{avg_bwtp_top5}, Avg_Fwt: {avg_fwt_top5} ")

    else:
        avg_end_acc = np.mean([acc[0] for acc in accuracy_list])  # Assuming acc[0] is TOP-1 accuracy
        avg_end_top5_acc = np.mean([acc[1] for acc in accuracy_list])  # Assuming acc[1] is TOP-5 accuracy
        print('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
        print("avg_end_acc {} avg_end_top5_acc {}".format(avg_end_acc, avg_end_top5_acc))
