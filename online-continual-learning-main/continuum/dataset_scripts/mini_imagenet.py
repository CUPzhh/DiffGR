...
class Mini_ImageNet(DatasetBase):
    ...
    def download_load(self):
        train_in = open("/media/lthpc/hd_auto/Liu/zhanghaohao/mini_imagenet/mini-imagenet-cache-train.pkl", "rb")
        train = pickle.load(train_in)
        train_x = train["image_data"].reshape([64, 600, 84, 84, 3])
        val_in = open("/media/lthpc/hd_auto/Liu/zhanghaohao/mini_imagenet/mini-imagenet-cache-val.pkl", "rb")
        val = pickle.load(val_in)
        val_x = val['image_data'].reshape([16, 600, 84, 84, 3])
        test_in = open("/media/lthpc/hd_auto/Liu/zhanghaohao/mini_imagenet/mini-imagenet-cache-test.pkl", "rb")
