...
class ContinualLearner(torch.nn.Module, metaclass=abc.ABCMeta):
    ...
    def evaluate(self, test_loaders):
        self.model.eval()
        acc_array = np.zeros((len(test_loaders), 2))  # 改为存储TOP-1和TOP-5
        ...
        def calculate_accuracy(output, target, topk=(1,)):
            ...
            return res  # 返回一个包含TOP-1和TOP-5准确率的列表
            ...
            for task, test_loader in enumerate(test_loaders):
                ...
                for i, (batch_x, batch_y) in enumerate(test_loader):
                    ...
                    if self.params.trick['ncm_trick'] or self.params.agent in ['ICARL', 'SCR', 'SCP']:
                        ...
                        logits = self.model.forward(batch_x)
                        top1_acc, top5_acc = calculate_accuracy(logits, batch_y, topk=(1, 5))
                    ...
                    if self.params.error_analysis:
                        ...
                        if task < self.task_seen-1:
                            ...
                            pass
                    acc1.update(top1_acc.item(), batch_y.size(0))
                    acc5.update(top5_acc.item(), batch_y.size(0))
                acc_array[task, 0] = acc1.avg()  # TOP-1
                acc_array[task, 1] = acc5.avg()  # TOP-5
        print(acc_array)
